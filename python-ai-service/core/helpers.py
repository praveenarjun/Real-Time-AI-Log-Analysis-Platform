...
def normalize_json_response(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "anomalies" in normalized and isinstance(normalized["anomalies"], list):
        for item in normalized["anomalies"]:
            if "sev" in item:
                item["severity"] = item["sev"]
            if "desc" in item:
                item["description"] = item["desc"]
            if "svc" in item:
                item["affected_service"] = item["svc"]

    return normalized


def truncate_logs(
    logs: List[Dict[str, Any]], max_count: int = 50
) -> List[Dict[str, Any]]:
...
def calculate_error_rate(logs: List[Dict[str, Any]]) -> float:
    ...
    if not logs:
        return 0.0
    error_count = sum(
        1 for log in logs if log.get("level", "").upper() in ["ERROR", "FATAL"]
    )
    return error_count / len(logs)


def group_logs_by_service(
