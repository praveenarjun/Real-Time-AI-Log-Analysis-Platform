...
class LogKafkaConsumer:
    ...
    async def consume_batches(self, batch_size: int, timeout_ms: int = 1000) -> AsyncGenerator[List[Dict[str, Any]], None]:
            ...
            while True:
                ...
                try:
                    ...
                    while len(batch) < batch_size:
                        remaining = (timeout_ms / 1000) - (asyncio.get_event_loop().time() - start_time)
                        if remaining <= 0:
                            break
                        
                        try:
                            msg = await asyncio.wait_for(self.consumer.getone(), timeout=remaining)
                            val = msg.value
