...
def analyze_error_trend(logs_json: str, service_name: str) -> str:
    ...
    try:
        ...
        minute_counts = {}
        for l in logs:
            if l.get("service_name") == service_name:
                ts = l.get("timestamp", "")[:16] # Minute precision
                if l.get("level", "").upper() in ["ERROR", "FATAL"]:
                    minute_counts[ts] = minute_counts.get(ts, 0) + 1
        
        if len(minute_counts) < 2:
            return json.dumps({"trend": "stable", "info": "Insufficient data for regression"})
        ...
        return json.dumps({
            ...
            "projected_critical_time": f"{round(float(projected_mins), 1)} mins" if projected_mins > 0 else "N/A",
...
def check_resource_trajectory(service_name: str) -> str:
    ...
    try:
        trajectories = {
            "memory": {"usage_trend": "+1.2%/min", "time_to_exhaustion": "45 mins", "recommendation": "Check for memory leak in /api/data"},
            "disk": {"usage_trend": "stable", "time_to_exhaustion": "14 days", "recommendation": "None"},
            "cpu": {"usage_trend": "+5%/min", "time_to_exhaustion": "12 mins", "recommendation": "Scale replicas to 3"}
        }
...
def match_failure_patterns(anomalies_json: str) -> str:
    ...
    patterns = [
        ...
        {"name": "Database Cascade", "progression": ["pool warning", "exhausted", "timeout", "total outage"]},
        {"name": "Network Partition", "progression": ["latency spike", "retries", "circuit-breaker-open", "503"]}
    ]
...
class PredictorAgent:
    def __init__(self, llm: ChatOpenAI):
        self.llm = llm
        self.tools = [analyze_error_trend, check_resource_trajectory, match_failure_patterns]
        self.llm_with_tools = self.llm.bind_tools(self.tools)

    def predict(self, log_entries: List[Dict[str, Any]], anomalies: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        risk_score = 0.1
        if len(anomalies) > 0:
            risk_score = 0.75 if any(a.get("severity") == "CRITICAL" for a in anomalies) else 0.45
        ...
        messages = [
            ...
            HumanMessage(content=f"Analyze the current system state and predict future failures: \n\n{json.dumps(context)}")
        ]
        ...
        try:
            ...
            logger.warning(f"AI Quota issue in Predictor. Switching to baseline risk model. Error: {e}")
