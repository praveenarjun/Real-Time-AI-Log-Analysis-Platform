...
class AlerterAgent:
    ...
    def decide(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        context = {
            ...
            "report_summary": incident_report.get(
                "executive_summary", "RCA pending..."
            ),
        ...
        messages = [
            ...
            HumanMessage(
                content=f"Decide the optimal alerting strategy for this incident: \n\n{json.dumps(context)}"
            ),
        ]
        ...
        try:
            ...
            return {
                "should_alert": decision.get(
                    "should_alert", context["risk_score"] > 0.4
                ),
                "channels": decision.get("channels", rule_base["channels"]),
                "message": decision.get(
                    "message", f"Incident detected in {incident_report.get('title')}"
                ),
                "urgency": decision.get("urgency", rule_base["urgency"]),
                "escalation_needed": decision.get(
            ...
            logger.warning(
                f"AI Offline for Alerter. Using local notification routing. Error: {e}"
            )
