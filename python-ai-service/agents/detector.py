...
from core.helpers import parse_json_response, truncate_logs, generate_log_summary
...
def check_log_patterns(logs_json: str) -> str:
    ...
    try:
        logs = json.loads(logs_json)
        if not logs: return json.dumps({"error": "Empty batch"})
        ...
        for l in logs:
            if l.get("level", "").upper() in ["ERROR", "FATAL"]:
                msg = l.get("message", "")[:100]
                svc = l.get("service_name", "unknown")
...
def check_frequency_anomaly(logs_json: str) -> str:
    ...
    try:
        logs = json.loads(logs_json)
        if len(logs) < 5: return json.dumps({"info": "Insufficient data for frequency analysis"})
        ...
        for l in logs:
            ts = l.get("timestamp", "")[:16] # ISO minute
            svc = l.get("service_name", "unknown")
...
def check_critical_errors(logs_json: str) -> str:
    ...
    crit_patterns = [
        "OutOfMemoryError", "StackOverflowError", "ConnectionRefused",
        "TimeoutException", "DeadlockException", "DiskFull", "PermissionDenied",
        "CertificateExpired", "SecurityBreach", "DataCorruption", "SegmentationFault"
    ]
...
def check_security_anomalies(logs_json: str) -> str:
    ...
    try:
        ...
        risk = "low"
        if len(issues) > 5: risk = "high"
        elif len(issues) > 0: risk = "medium"
...
class DetectorAgent:
    ...
    def detect(self, log_entries: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        critical_logs = [l for l in log_entries if isinstance(l, dict) and l.get("level", "").upper() in ["FATAL", "CRITICAL"]]
        if critical_logs:
            logger.warning(f"TOKEN SAVER: {len(critical_logs)} FATAL/CRITICAL logs found. Bypassing AI Detection to save quota.")
            anomalies = []
        ...
        context = {"logs": log_entries[:20]}
        
        # 1. Sanitize and Truncate
        valid_logs = []
        has_critical = False
        for l in log_entries:
            if isinstance(l, dict) and "message" in l:
                valid_logs.append(l)
                if str(l.get("level", "")).upper() in ["FATAL", "CRITICAL"]:
                    has_critical = True
        ...
        messages = [
            ...
            HumanMessage(content=f"Analyze this telemetry batch for anomalies: {prompt_hint}\n\n{logs_json}")
        ]
        ...
        try:
            ...
            if has_critical:
                logger.warning("Agent check bypassed: FATAL/CRITICAL logs detected. Forcing Forensic Pipeline.")
                is_anomaly = True
                severity = "CRITICAL"
                anoms = final_data.get("anomalies", [])
                for l in log_entries:
                    if isinstance(l, dict) and str(l.get("level", "")).upper() in ["FATAL", "CRITICAL"]:
                        anoms.append({
                            "type": "CRITICAL_LOG",
                            "service": l.get("service_name"),
                            "message": l.get("message"),
            ...
            logger.warning(f"AI Quota/Connection issue in Detector. Handing over to Local Safe-Mode. Error: {e}")
