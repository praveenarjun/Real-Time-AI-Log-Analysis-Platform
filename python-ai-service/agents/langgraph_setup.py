...
class LogAnalysisSupervisor:
    def __init__(self):
        ...
        if provider == "gemini":
            ...
            logger.info(f"Using Azure OpenAI (Standard): {settings.LLM_MODEL}")
        ...
        try:
            ...
            for i, log in enumerate(entries):
                if isinstance(log, dict):
                    logger.info(
                        f"DEBUG: LogEntry[{i}] -> Level: {log.get('level')}, Service: {log.get('service_name')}, Msg: {log.get('message')[:50]}"
                    )

            total = len(entries)
            errors = sum(
                1
                for log_item in entries
                if isinstance(log_item, dict)
                and str(log_item.get("level", "")).upper()
                in ["ERROR", "FATAL", "CRITICAL"]
            )
            services = list(
                set(
                    log_item.get("service_name", "unknown")
                    for log_item in entries
                    if isinstance(log_item, dict)
                )
            )
    ...
    def _detect_node(self, state: LogAnalysisState):
        ...
        if state.get("ai_circuit_broken"):
            logger.warning(
                "CIRCUIT BREAKER: Skipping AI Detector due to earlier quota error."
            )
            return {
        ...
        try:
            ...
            logger.info(
                f"Detector Results [HasAnomaly: {results.get('has_anomalies')}, Severity: {severity}]"
            )
            return {
            ...
            if is_429:
                logger.warning(
                    "DETECTOR FALLBACK: AI limit reached. Activating Circuit Breaker."
                )
                return {
                    "anomalies_detected": True,
                    "anomaly_severity": "info",
            ...
            fatal_logs = [
                ...
                for log_item in entries
                if isinstance(log_item, dict)
                and str(log_item.get("level", "")).upper() in ["FATAL", "CRITICAL"]
            ]
            if fatal_logs:
                ...
                return {
                    ...
                    "anomalies": [
                        {
                            "type": "CRITICAL_LOG",
                            "message": fatal_logs[0].get("message"),
                        }
                    ],
    ...
    def _analyze_node(self, state: LogAnalysisState):
        ...
        if state.get("ai_circuit_broken"):
            ...
            return {
                "root_cause": "Circuit Breaker Active (AI Quota Over)",
                "affected_services": [],
                "ai_circuit_broken": True,
            }

        try:
            results = self.analyzer_agent.analyze(
                state["log_entries"], state["anomalies"]
            )
            ...
            if is_429:
                logger.warning(
                    "ANALYZER FALLBACK: AI limit reached. Activating Circuit Breaker."
                )
                return {
                    "root_cause": "Analysis Failure (AI Quota Limit)",
                    "ai_circuit_broken": True,
                }
    ...
    def _predict_node(self, state: LogAnalysisState):
        ...
        try:
            results = self.predictor_agent.predict(
                state["log_entries"], state["anomalies"]
            )
            return {
            ...
            if is_429:
                logger.warning(
                    "PREDICTOR FALLBACK: AI limit reached. Activating Circuit Breaker."
                )
                return {
                    "risk_score": 0.7,
            ...
            logger.warning(
                f"PREDICTOR FALLBACK: Quota reached. Using baseline risk model. Error: {e}"
            )
            return {
                "predictions": [
                    {
                        "forecast": "Continued instability expected (Safe-Mode)",
                        "probability": 0.85,
                    }
                ],
                "risk_score": 0.9,
    ...
    def _report_node(self, state: LogAnalysisState):
        ...
        try:
            ...
            if is_429:
                logger.warning(
                    "REPORTER FALLBACK: AI limit reached. Activating Circuit Breaker."
                )
                return {
                    "incident_report": {
                        "summary": "Analysis Quota Exceeded. Incident details generated from local fingerprints."
                    },
                    "ai_circuit_broken": True,
                }
    ...
    def _alert_node(self, state: LogAnalysisState) -> Dict[str, Any]:
        ...
        try:
            ...
            if is_429:
                logger.warning(
                    "ALERTER FALLBACK: AI limit reached. Finalizing locally."
                )
                return {
                    "should_alert": True,
                    "alert_channels": ["log"],
